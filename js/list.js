$(function(){
	...
	$.ajax({
 			...
 			$('.small').click(function(){
				...
				/*$('#connect_box a').remove().find('.price_num').sort(function(a,b){
					a.innerText.match(/\d+/) - b.innerText.match(/\d+/);
				}).each(function(){
					$(this).parents('.select').appendTo('#connect_box');
				})*/
 			});
 			$('.big').click(function(){
	...
	function godeaddead(i,parts,arr,k){
		parts.append($(`<a href="#" class="select" title=${arr[i].id}><div class="parts_box_connect">
							...
							<div class = "price">
								<div class="discount-price"><span>¥</span><span class="price_num">${arr[i].price}</span></div>
							</div>
						</div></a>`));
